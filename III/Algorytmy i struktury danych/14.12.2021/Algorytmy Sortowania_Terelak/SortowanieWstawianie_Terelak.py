...
def sortowanieWstawianie(T):
    for k in range(1, len(T)):
        ...
        i = k - 1
        while i >= 0 and tmp < T[i]: # Elementy o większej wartości od obecnie sprawdzanego przesuń o 1
            T[i+1] = T[i] 
            i -= 1
        T[i+1] = tmp # Po przesunięciu większych elementów wstaw wartość elementu obecnie sprawdzanego
        #print("Krok {0}: Przenoszenie elementu {1} z pozycji {2} na pozycję {3}".format(k, tmp, k, i+1))
        #print(T)
